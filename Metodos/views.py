    ...
    if request.method == 'POST':
        try: 
            ...
            if "errors" in datos and datos["errors"]:
                context['error_message'] = f'There was an error in the False Position method in: {datos["errors"]}'
            ...
            if "results" in datos:
                ...
                if datos.get("root") is not None:
                    fig.add_trace(go.Scatter(x=[float(datos["root"])], y=[float(0)], mode='markers', name='Root found'))

                fig.update_layout(title=f'Function: {fx} in interval [{xi_copy}, {xs_copy}]',
                                    xaxis_title='x',
                                    yaxis_title='f(x)')
                ...
                context = {'df': df.to_html(), 'plot_html': plot_html, 'mensaje': f'The solution is: {datos["root"]}'}
        except Exception as e:
            context = {'error_message': f'There was an error in the False Psition method in: {str(e)}'}
            return render(request, 'error.html', context)
...
def reglaFalsa(a, b, Niter, Tol, fx, tipo_error):
    ...
    try:
        if Fun.subs(x, a)*Fun.subs(x, b) > 0:
            output["errors"].append("The function does not change sign in the given interval")
            return output
        while (error > cond) and (i < Niter):
            if i == 1:
                ...
                Fx_3 = Fun.subs(x, xm)
                Fx_3 = Fx_3.evalf()
                datos.append([i, '{:^15.10f}'.format(a), '{:^15.10f}'.format(xm), '{:^15.10f}'.format(b), '{:^15.2E}'.format(Fx_3)])
            else:
...
def puntoFijoView(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if datos.get("root") is not None:
                fig.add_trace(go.Scatter(x=[float(datos["root"])], y=[0], mode='markers', name='Root found'))

            fig.update_layout(title=f'Function: {fx} and {gx} in interval [{X0 - 5}, {X0 + 5}]',
                            xaxis_title='x',
                            yaxis_title='f(x) / g(x)')
            ...
            context = {'error_message': f'There was an error in the Fixed Point method at: {str(e)}'}
            return render(request, 'error.html', context)
...
def bisection(request):
    if request.method == 'POST':
        ...
        if xi is None or xs is None or tol is None or niter is None or funcion is None or tipo_error is None:
            ...
            funcion = 'x**2 - 3'
            tipo_error = 'absoluto'
            mensaje+="The default case was taken, please enter the values ​​for all fields <br> "
        else:
            xi=float(xi)
            xs=float(xs)
            tol=float(tol)
        ...
        try:
            ...
            if fi==0:
                ...
                if(fxm==0):
                    mensaje+="The solution is: "+str(xm)
                    hay_solucion=True
                    solucion=xm
                    fsolucion=fxm
                    ...
                    if fxm==0:
                        ...
                        if error<tol:
                            mensaje+="The approximate solution that meets the tolerance is : "+str(xm)
                            hay_solucion=True
                            solucion=xm
                            fsolucion=fxm
                        else:
                            mensaje+="The maximum number of iterations has been reached"

            else:
                mensaje+="There is no root in the interval, try another interval"
            ...
            if hay_solucion:
                fig=plot_fx_puntos(funcion,xi_copy,xs_copy,[str(solucion)],[str(fsolucion)],'Root found')
            else:
                fig=fx_plot(funcion,xi_copy,xs_copy)
            plot_html = fig.to_html(full_html=False, default_height=500, default_width=700)
            dataframe_to_txt(df,"bisection")
            plot_to_png(fig,"bisection")
            context = {'df': df.to_html(), 'plot_html': plot_html}
            mensaje=mark_safe(mensaje)
            context['mensaje']=mensaje
            context['nombre_metodo']="Bisection"
            return render(request, 'one_method.html', context)
        except:
            mensaje+="Error in the data entered, please verify that the data is correct"
            context = {'mensaje':mensaje}
            return render(request, 'one_method.html', context)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if f0==0:
                ...
                absoluto=abs(x2-x1)
                if f2==0:
                    mensaje+="The solution is: "+str(x2)
                    hay_solucion=True
                    solucion=x2
                    fsolucion=f2
                    ...
                    while itera < niter and tol < error and f1!=0 and f0!=0:
                        ...
                        if tipo_error=="absoluto":
                            ...
                            error=relativo
                    if f1==0:
                        mensaje+="The solution is: "+str(x1)
                        hay_solucion=True
                        solucion=x1
                        fsolucion=f1
                    if error<tol:
                        mensaje+="The approximate solution that meets the tolerance is : "+str(x2)+" in the iteration "+str(itera)+" = x"+str(itera)
                        hay_solucion=True
                        solucion=x2
                        fsolucion=f2
                    elif itera==niter:
...
def newton(x0, Tol, Niter, fx, df, tipo_error):
    ...
    try:
        datos.append([c, '{:^15.10f}'.format(x0), '{:^15.10f}'.format(f)])
        ...
        while Error > Tol and f != 0 and derivada != 0 and c < Niter:
            ...
            datos.append([c, '{:^15.10f}'.format(float(xi)), '{:^10.5E}'.format(
                float(f)), '{:^10.5E}'.format(float(Error))])
    ...
    if request.method == 'POST':
        try:
            ...
            if "results" in datos:
                ...
                fig.update_layout(title=f'Function: {fx} in interval [{xi_copy - 10}, {xs_copy + 10}]',
                                xaxis_title='x',
                                yaxis_title='f(x)')
            ...
            return render(request, 'one_method.html', context)
                ...
                context = {'error_message': f'There was an error in Newton method in: {str(e)}'}
                return render(request, 'error.html', context)
...
def raicesMultiples(fx, x0, tol, niter, tipo_error):
    ...
    if d_ex2 == 0 or d2_ex2 == 0:
        output["errors"].append("Error: The first or second derivative evaluated at x0 is zero.")
        print('entro')
        return output
    ...
    try:
        while (error > tol) and (i < niter):  # Se repite hasta que el intervalo sea lo pequeño que se desee
            if i == 0:
                ...
                if d_ex2 == 0 or d2_ex2 == 0:
                    output["errors"].append("Error: The first or second derivative evaluated during the iteration is zero.")
                    return output
        ...
        output["errors"].append(f"Error in the data: {str(e)}")
        return output
...
def raicesMultiplesView(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if "errors" in datos and datos["errors"]:
                context = {'error_message': f'there was an error in Newton method at: {str(datos["errors"])}'}
                return render(request, 'error.html', context)
            ...
            return render(request, 'one_method.html', context)
                ...
                context = {'error_message': f'there was an error in Newton method at: {str(e)}'}
                return render(request, 'error.html', context)
...
def iterativos(request):
    if request.method == 'POST':
        ...
        try:
            ...
            context={'mensaje':'The operation could not be performed'}
            return render(request,'resultado_iterativo.html',context)
...
def interpolacion(request):
    if request.method == 'POST':
        try:
            ...
            n = len(xi)
            if n<=1:
                context = {'error_message': f'2 or more points must be entered'}
                return render(request, 'error.html', context)
            for i in range(n-1):
                if xi[i] >= xi[i+1]:
                    context = {'error_message': f'The x values ​​must be ordered from smallest to largest.'}
                    return render(request, 'error.html', context)
            ...
            if metodo_interpolacion == 'lagrange':
                try:
                    ...
                    fig.update_layout(title=f'Function: {polisimple}',
                                        xaxis_title='x',
                                        yaxis_title='f(x)')
                    ...
                    context = {'error_message': f'There was an error with lagrange in: {str(e)}'}
                    return render(request, 'error.html', context)
                ...
                try:
                    ...
                    newton_poly_df = pd.DataFrame({'Polynomial': [str(newton_poly)]})
                    ...
                    fig.add_trace(go.Scatter(x=X, y=Y, mode='markers', name='Original points'))

                    # Agregar el polinomio de Newton
                    fig.add_trace(go.Scatter(x=x_vals, y=y_vals, mode='lines', name='Newton polynomial'))

                    fig.update_layout(title='Newton Interpolation with Divided Differences',
                                    xaxis_title='X',
                                    yaxis_title='Y')
                    ...
                    context = {
                        ...
                        'nombre_metodo': 'Newton Divided Differences', 
                        'plot_html': plot_html
                    }
                    ...
                    context = {'error_message': f'There was an error with Newton Divided Differences in: {str(e)}'}
                    return render(request, 'error.html', context)
                ...
                try:
                    ...
                    fig=plot_fx_puntos(pol,xi[0],xi[-1],xi,fi,'point given')
                    plot_html = fig.to_html(full_html=False, default_height=500, default_width=700)
                    plot_to_png(fig,"vandermonde")
                    text_to_txt(pol,"vandermonde_pol")
                    
                    mensaje = "A solution was found"
                    context = {
                        'vandermonde': True,
                        'ma': mA_html,
                        'vb': vb_html,
                    ...
                    context = {'error_message': f'there was an error with vandermonde: {str(e)}'}
                    return render(request, 'error.html', context)
                ...
                coef_df = pd.DataFrame(coef_list, columns=['Slope (m)', 'Intercept (b)'])
                coef_df_to_html = coef_df.to_html(classes='table table-striped', index=False)

                # Crear DataFrame para las funciones
                func_df = pd.DataFrame({'Function': [str(func) for func in px_tabla]})
                ...
                for i, (x_vals, y_vals) in enumerate(funciones_evaluadas):
                    fig.add_trace(go.Scatter(x=x_vals, y=y_vals, mode='lines', name=f'stretch {i+1}'))
                
                fig.update_layout(title='Piecewise Cubic Spline',
                                xaxis_title='X',
                                yaxis_title='Y')
            ...
            context = {'error_message': f'An error occurred while obtaining the data, fill in all the fields correctly, whole numbers or decimals'}
            return render(request, 'error.html', context)
