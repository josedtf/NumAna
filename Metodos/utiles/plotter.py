...
def plot_fx_puntos(funcion,xinicial,xfinal,puntosx,puntosy,alias_punto):
    ...
    fig.update_layout(title=f'Function: {funcion} in interval [{xinicial}, {xfinal}]',
            xaxis_title='x',
            yaxis_title='f(x)')
    return fig
...
def spline_plot(fxs,puntosx,puntosy):
        try:
                ...
                fig.update_layout(title=f'Spline function',
                        xaxis_title='x',
                        yaxis_title='f(x)')
                fig.add_trace(go.Scatter(x=puntosx, y=puntosy, mode='markers', name='Points given'))
                return fig
